...
class tictactoe extends JFrame implements ItemListener, ActionListener{  
	...
	public void itemStateChanged(ItemEvent e){  
		System.out.println("fdgfdh");
		System.out.println(c1.getState()+" "+c2.getState());
		if(c1.getState()){   
			type=false;  
		}  
	...
	/********************************/  
		if(type==true){//logicfriend  
			if(e.getSource()==reset){
				for(i=0;i<=8;i++) {b[i].setIcon(null);}//eof for    
			}  
			else{   
				for(i=0;i<=8;i++){  
					if(e.getSource()==b[i]){  
			...
			if(e.getSource()==reset){
	            ...
	            for(i=0;i<=8;i++){  
	               if(e.getSource()==b[i]){  
	                  if(b[i].getIcon()==null){   
	                	  b[i].setIcon(ic1);    
	                	  if(b[4].getIcon()==null){b[4].setIcon(ic2);this.check(5);}
